...
Page({
  ...
  contentMoveing (e) {
    ...
    var dragLengthX = (pageX - e.touches[0].pageX) * dragScaleP   // 2 移动时 手势位移与 实际元素位移的比
    var dragLengthY = (pageY - e.touches[0].pageY) * dragScaleP
    
    // 这里取两个最小值是为了保证 裁剪框在屏幕视野内，应为触模点实际的移动范围要比裁剪框移动范围大的多。是整个屏幕的区域
    ...
    // console.log("最新的pagex",e.touches[0].pageX)
    pageX = e.touches[0].pageX
    pageY = e.touches[0].pageY
  },
  ...
  getImageInfo () {
    ...
    wx.showLoading({
      title: '图片生成中...',
        ...
        ctx.draw()
        // 获取画布要裁剪的位置和宽度   均为百分比 * 画布中图片的宽度    保证了在微信小程序中裁剪的图片模糊  位置不对的问题 位置不对的问题
